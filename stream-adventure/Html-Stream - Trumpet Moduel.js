...
var loudHTML = tr.createStream('.loud'); 
//<-----tr.createStream This is creating a readable and 
//writable stream. 
...
loudHTML.on('data',function(chunk){
	loudHTML.write(chunk.toString().toUpperCase());
});

loudHTML.on('end', function(chunk){
	loudHTML.end();
});
